...
public class HydraPower extends AbstractPower implements CloneablePowerInterface, InvisiblePower, CreatureValuePower {
    public AbstractCreature source;

    public static final String POWER_ID = LeshyMod.makeID(HydraPower.class.getSimpleName());
    ...
    public HydraPower(final AbstractCreature owner, final AbstractCreature source, final int amount) {
        name = NAME;
        ID = POWER_ID;
    ...
    public AbstractPower makeCopy() {
        return new HydraPower(owner, source, amount);
    }

    @Override
    public int giveAttack(AbstractCreatureCard c) {
        if(c instanceof Hydra)
            return this.amount*Hydra.HYDRA_BUFF;
        return 0;
    }
